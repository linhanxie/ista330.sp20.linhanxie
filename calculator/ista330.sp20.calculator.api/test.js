...
it(’should return 31 when the expression is "3+2**5+8/4-2*3"’, function() { assert.equal(calculator.calculate(’3+2**5+8/4-2*3’), 31);
});
...
it(’should return NaN when the expression is "3-3*1/3-0*2/0"’, function() { assert.equal(isNaN(calculator.calculate(’3-3*1/3-0*2/0’)), true);
}); });
